...
export const Select = forwardRef(({ className, id, placeholder, label, error, data, ...props}: SelectProps, forwardRef) => {
        ...
        <div className={styles.root}>
            ...
            <div className={styles.container}>
                <select id={id} className={selectClass} {...props}>
                    ...
                    {data.map((item, index) => <option className={styles.option} key={index} id={item.id} value={item.value}>{item.label}</option>)}
                </select>
