...
export const Stepper: React.FC<PropsWithChildren<StepperProps>> & {
  Step: typeof Step;
} = ({ children, active, onStepChange, allowNextStepsSelect = true }) => {
      ...
      <div>
        ...
        <div className={styles.indexSteps}>
          {_children.map((_, index) => (
            <div
              key={index}
              className={index <= active ? styles.index_active : ""}
            >
              {index + 1}
            </div>
          ))}
